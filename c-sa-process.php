...
if($checkin1 > $checkout2){
             ...
             ?>
              <meta http-equiv="refresh" content="0;URL='./?page_id=c-sa&room_id=<?php echo $roomid;?>'>" /> 
<?php
    ...
    //$query = mysqli_query($databaseconnection,"SELECT room_no FROM transaction WHERE $roomid = room_id AND '$checkin' BETWEEN checkin and checkout AND '$checkout'  BETWEEN checkin and checkout");
    //$count = mysqli_num_rows($query);
    
    
    //$sql = mysqli_query($databaseconnection,"SELECT room_number FROM available_rooms INNER JOIN transaction ON  available_rooms.room_id=transaction.room_id  WHERE available_rooms.room_id = $roomid AND available_rooms.room_number != transaction.room_no AND '$checkin' NOT BETWEEN transaction.checkin and transaction.checkout AND '$checkout' NOT BETWEEN transaction.checkin and transaction.checkout");
    ...
    //$query1 = mysqli_query($databaseconnection, "SELECT * FROM available_rooms WHERE $roomid = room_id AND availability = 'Available' ");
    ...
    ?>
    		...
    		<meta http-equiv="refresh" content="0;URL='./?page_id=c-available-rooms&room_id=<?php echo $roomid;?>'" />
    <?php
    }else{
